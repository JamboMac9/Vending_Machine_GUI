...
/**
 * Card customer class, designed to reduce the balance
 * a customer is left with on their loyalty card
...
public class CardCustomer {
	...
	BigDecimal balance = new BigDecimal(Double.parseDouble(VendingMachineGUI.getChangeTf()));
	double sp = 0;

	public void cardProduct() {

		if(VendingMachineGUI.getCokeRb().isSelected()) {
			cokeDiscount();
			// displays the new balance to the customer once coke has been purchased
			VendingMachineGUI.changeTf.setText(String.format("%.2f", balance));
	...
	protected void cokeDiscount() {
		// setting the price set to coke, including the "£"
		double discount = Double.valueOf(VendingMachineGUI.getCokePrice().getText().substring(1));
		sp = discount;
		// if a product has been purchased
		if (SystemController.checkBalance(balance, sp)) {
			// subtract the cost of the coke from the balance
			balance = balance.subtract(BigDecimal.valueOf(discount)).setScale(2, RoundingMode.HALF_EVEN);
			JOptionPane.showMessageDialog(null, "COKE - Product Dispensed"); 
		} else
			JOptionPane.showMessageDialog(null, "Not enough money inserted");
	}

	protected void lemonDiscount() {

		double discount = Double.valueOf(VendingMachineGUI.getLemonPrice().getText().substring(1));
		sp = discount;
		// if a product has been purchased
		if (SystemController.checkBalance(balance, sp)) {
	...
	protected void tangoDiscount() {

		double discount = Double.valueOf(VendingMachineGUI.getTangoPrice().getText().substring(1));
		sp = discount;
		// if a product has been purchased
		if (SystemController.checkBalance(balance, sp)) {
	...
	protected void waterDiscount() {

		double discount = Double.valueOf(VendingMachineGUI.getWaterPrice().getText().substring(1));
		sp = discount;
		// if a product has been purchased
		if (SystemController.checkBalance(balance, sp)) {
	...
	protected void spriteDiscount() {

		double discount = Double.valueOf(VendingMachineGUI.getSpritePrice().getText().substring(1));
		sp = discount;
		// if a product has been purchased
		if (SystemController.checkBalance(balance, sp)) {
	...
	protected void pepsiDiscount() {

		double discount = Double.valueOf(VendingMachineGUI.getPepsiPrice().getText().substring(1));
		sp = discount;
		// if a product has been purchased
		if (SystemController.checkBalance(balance, sp)) {
